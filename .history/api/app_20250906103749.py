...
class EmailScoreRequest(BaseModel):
    ...
    subject: str
    body: str
    mode: Optional[str] = "llm"  # Only LLM mode supported (Gemini)
...
async def score_email(request: EmailScoreRequest):
    ...
    try:
        ...
        # Skip ML classification (Gemini-only mode)
        response.model_info["ml"] = "disabled"
        
        # Gemini LLM classification (main method)
        if llm_client and request.mode == "llm":
            try:
                ...
                if llm_result:
                    ...
                    response.llm_explanation = "Gemini classification failed"
            ...
            response.llm_label = "unavailable"
            response.llm_confidence = 0.0
            response.llm_explanation = "Gemini client not available"
            response.model_info["llm"] = "unavailable"
