...
class EmailAgent:
    def __init__(self, config: Config):
        ...
        self.ml_inference = MLInference(config.ml_artifacts_path)
        
        # Initialize LLM client if available and mode requires it
        self.llm_client = None
        if config.mode in ['llm', 'hybrid'] and LLM_AVAILABLE:
            try:
                from client import LLMClient
                ...
                self.logger.info(f"LLM client initialized with model: {config.openai_model}")
            except Exception as e:
                self.logger.error(f"Failed to initialize LLM client: {e}")
                if config.mode == 'llm':
                    raise
        elif config.mode in ['llm', 'hybrid'] and not LLM_AVAILABLE:
            self.logger.warning("LLM mode requested but LLM client not available")
            if config.mode == 'llm':
                raise ImportError("LLM mode requires OpenAI client")
    ...
    def process_email(self, email_data: Dict) -> EmailEvent:
        ...
        # ML inference
        ml_prediction, ml_score = self.ml_inference.predict(subject, body)
        ...
        if self.llm_client and self.config.mode in ['llm', 'hybrid']:
            try:
                llm_start = time.time()
