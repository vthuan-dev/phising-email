...
# Kiểm tra SPAM folder với encoding đúng
spam_folders = ['[Gmail]/Thư rác', '[Gmail]/Spam', 'Spam', 'Junk']
for folder_name in spam_folders:
    try:
        # Encode folder name properly
        folder_encoded = folder_name.encode('utf-7').decode('ascii')
        status, messages = mail.select(folder_encoded)
        if status == 'OK':
            num_messages = int(messages[0])
            print(f"\n{folder_name}: {num_messages} emails")
            
            # Lấy 3 email mới nhất
            if num_messages > 0:
                start = max(1, num_messages - 2)
                end = num_messages
        ...
        # Đóng folder trước khi chuyển sang folder khác
        try:
            mail.close()
        except:
            pass
